...
public class BoomboxItem extends Item implements ContainerItem {
    ...
    private static final Component PAUSE = Component.translatable("item." + EngravedMod.MOD_ID + ".boombox.pause", Component.keybind("key.sneak"), Component.keybind("key.use")).withStyle(ChatFormatting.GRAY);
    private static final Component RECORDS = Component.translatable("item." + EngravedMod.MOD_ID + ".boombox.records");
    public static final Component PAUSED = Component.translatable("item." + EngravedMod.MOD_ID + ".boombox.paused").withStyle(ChatFormatting.YELLOW);
    ...
    private static boolean canAdd(ItemStack boombox, ItemStack record) {
        if (!(boombox.is(EngravedMod.BOOMBOX_ITEM)) || !(record.getItem() instanceof PlayableRecord))
            return false;

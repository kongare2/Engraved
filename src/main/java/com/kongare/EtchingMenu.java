...
public class EtchingMenu extends AbstractContainerMenu {
    ...
    public static final ResourceLocation EMPTY_SLOT_MUSIC_DISC = new ResourceLocation(EngravedMod.MOD_ID, "item/empty_etching_table_slot_music_disc");
    public static final ResourceLocation EMPTY_SLOT_MUSIC_LABEL = new ResourceLocation(EngravedMod.MOD_ID, "item/empty_etching_table_slot_music_label");
    ...
    public EtchingMenu(int id, Inventory inventory, ContainerLevelAccess containerLevelAccess) {
        super(EngravedMod.ETCHING_MENU, id);
        ...
        this.discSlot = this.addSlot(new Slot(this.input, 0, 44, 43) {
            ...
            public boolean mayPlace(ItemStack stack) {
                return stack.getItem() == EngravedMod.BLANK_MUSIC_DISC || stack.getItem() == EngravedMod.ENGRAVED_MUSIC_DISC;
            }
            ...
            public void onTake(Player player, ItemStack stack) {
                ...
                containerLevelAccess.execute((level, pos) -> {
                    ...
                    if (EtchingMenu.this.lastSoundTime != l) {
                        level.playSound(null, pos, EngravedMod.UI_ETCHER_TAKE_RESULT, SoundSource.BLOCKS, 1.0F, 1.0F);
    ...
    public boolean stillValid(Player player) {
        return stillValid(this.access, player,EngravedMod.ETCHING_TABLE);
    }

    @Override
    public boolean clickMenuButton(Player player, int index) {
        if (index >= 0 && index < EngravedMusicDiscItem.LabelPattern.values().length) {
    ...
    public void slotsChanged(Container container) {
        ...
        if (resultStack.isEmpty() && labelStack.isEmpty()) {
            if (!discStack.isEmpty() && discStack.getItem() == EngravedMod.ENGRAVED_MUSIC_DISC) {
                this.labelIndex.set(EngravedMusicDiscItem.getPattern(discStack).ordinal());
            } else {
    ...
    private void sendtoplayerInvalidUrl(ServerPlayer player, String msg) {
        FriendlyByteBuf pkt = PacketByteBufs.create().writeUtf(msg);
        ServerPlayNetworking.send(player, EngravedMod.INVALID_URL_PACKET, pkt);
    }
    ...
    private void setupResultSlot() {
        ...
        sendtoplayerInvalidUrl((ServerPlayer) this.player,"");
        this.resultSlot.set(ItemStack.EMPTY);
        if (this.labelIndex.get() >= 0 && this.labelIndex.get() < EngravedMusicDiscItem.LabelPattern.values().length) {
            ItemStack discStack = this.discSlot.getItem();
            ItemStack labelStack = this.labelSlot.getItem();

            if (discStack.getItem() == EngravedMod.ENGRAVED_MUSIC_DISC || (!discStack.isEmpty() && !labelStack.isEmpty())) {
                ...
                this.currentRequest = CompletableFuture.supplyAsync(() -> {
                    ItemStack resultStack = new ItemStack(EngravedMod.ENGRAVED_MUSIC_DISC);
                    ...
                    if (discStack.getItem() == EngravedMod.ENGRAVED_MUSIC_DISC) {
                        discColor = EngravedMusicDiscItem.getDiscColor(discStack);
                        primaryLabelColor = EngravedMusicDiscItem.getLabelPrimaryColor(discStack);
                        secondaryLabelColor = EngravedMusicDiscItem.getLabelSecondaryColor(discStack);
                    ...
                    if (SoundSourceManager.isValidUrl(this.url)) {
                        try {
                            ...
                            if (!this.player.level.isClientSide())
                                sendtoplayerInvalidUrl((ServerPlayer) this.player,e instanceof CompletionException ? e.getCause().getMessage() : e.getMessage());
                        ...
                        try {
                            ...
                            if (!this.player.level.isClientSide())
                                sendtoplayerInvalidUrl((ServerPlayer) this.player,"Unknown host: " + this.url);
                            throw new CompletionException("Invalid URL", e);
                        } catch (Exception e) {
                            if (!this.player.level.isClientSide())
                                sendtoplayerInvalidUrl((ServerPlayer) this.player,e.getLocalizedMessage());
                            throw new CompletionException("Invalid URL", e);
                        }
                    }
                    ...
                    EngravedMusicDiscItem.setMusic(resultStack, data);
                    EngravedMusicDiscItem.setColor(resultStack, discColor, primaryLabelColor, secondaryLabelColor);
                    EngravedMusicDiscItem.setPattern(resultStack, EngravedMusicDiscItem.LabelPattern.values()[this.labelIndex.get()]);
